...
read_checkred_packages_raw <- function() {
  ...
  header <- stringi::stri_trim_both(unlist(lapply(xml2::xml_find_all(xml2::read_html(rrr[1]), "//th"), xml2::xml_text)))

  result_raw <- matrix(xml2::xml_text(xml2::xml_find_all(xml2::read_html(paste(rrr[2:length(rrr)], collapse = "\n")), "//td"), trim = TRUE),
                       ncol = length(header),
