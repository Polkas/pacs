...
- **LIFE DURATION**, life duration of the version where we should expect at least 14 days; `pacs::pac_timemachine("keras")` or `pacs::pac_lifeduration("keras)`.
...
`utils::news(package = "keras")`; e.g.  https://cran.r-project.org/package=keras/news/news.html.
- **REPO ACTIVITY**  how quick issues are answered and how often new commits are added; e.g. https://github.com/rstudio/keras.
- **SCOPE**, changes in DECRIPTION/NAMESPACE files between current and the new version; `pacs::pac_compare_versions("keras")` and `pacs::pac_compare_namespace("keras")`.
...
```{nomnoml, fig.width=16, fig.height=7, svg=FALSE}
#stroke: black
#padding: 0
#spacing: 0
#fontSize: 12
...
- **BUILT**, packages installed with a previous version of R could not work correctly with the new version of R. The library is sometimes persisted after an R update, mostly for a minor (last number) version update.
