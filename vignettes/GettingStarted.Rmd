...
At the moment of writing it the first invoke returns 3 dependencies whereas the second one 1405 and the last one 1427. It should be clear that when extending the scope (and recursively) with the `"Suggests"` field then the number of dependencies is exploding.
...
```r
pacs::app_deps(system.file("examples/04_mpg", package = "shiny"))
```
